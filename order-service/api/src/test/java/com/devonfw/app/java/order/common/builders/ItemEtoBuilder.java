...
public class ItemEtoBuilder {
	...
	private List<P<ItemEto>> parameterToBeApplied;
	...
	public ItemEtoBuilder name(final String name) {
		this.parameterToBeApplied.add(new P<ItemEto>() {
			@Override
			public void apply(ItemEto target) {
				target.setName(name);
			}
		});
		return this;
	...
	public ItemEto createNew() {
		...
		for (P<ItemEto> parameter : parameterToBeApplied) {
			parameter.apply(itemeto);
		}
		return itemeto;
	}
